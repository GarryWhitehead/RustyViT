...
fn build_cuda() {
    ...
    println!("cargo:rerun-if-changed=/cuda/kernels/vision");
    println!("cargo:rerun-if-changed=/cuda/kernels/tensor");

    // Glob for all .cu files.
    // TODO: Will need to do this for .cuh files when there are any.
    let out_dir = std::env::var("OUT_DIR").unwrap();
    let kernel_paths: Vec<PathBuf> = glob::glob("cuda/kernels/**/*.cu")
        .unwrap()
...
fn build_vulkan() {
    ...
    let out_dir = std::env::var("OUT_DIR").unwrap();
    let vision_glsl_paths: Vec<PathBuf> = glob::glob("vulkan/shaders/vision/*.glsl")
        .unwrap()
        .map(|path| path.unwrap())
        .collect();
    vision_glsl_paths
    ...
    shader_map.iter().for_each(|entry| {
        println!(
            "cargo:rerun-if-changed=vulkan/shaders/tensor/{}.glsl",
            entry.0
        )
    });
    ...
    shader_map.iter().for_each(|entries| {
        entries.1.iter().for_each(|entry| {
            let shader_path = format!("shaders/tensor/{}.glsl", entries.0);
