...
pub(crate) trait KernelOp<T: DataElem> {
...
impl<T: DataElem> MatMulKernel<T> for Vulkan
where
    Self: KernelOp<T>,
{
    fn forward(
        ...
        let out = Self::try_alloc(self, out_sz).unwrap();
        let matrix_c = Self::try_alloc(self, out_sz).unwrap();
        ...
        program
            .try_bind_ssbo::<T>("m_a", &lhs.slice(..).unwrap())
            .unwrap();
        program
            .try_bind_ssbo::<T>("m_b", &rhs.slice(..).unwrap())
            .unwrap();
        program
            .try_bind_ssbo::<T>("m_c", &matrix_c.slice(..).unwrap())
            .unwrap();
        program
            .try_bind_ssbo::<T>("m_d", &out.slice(..).unwrap())
            .unwrap();
