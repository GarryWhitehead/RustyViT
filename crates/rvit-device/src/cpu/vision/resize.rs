...
impl<T: PixelType, I: InterpMode> ResizeKernel<T, I> for Cpu
where
    Self: InterpOp<I>,
{
    fn resize(
        ...
        let mut rz_img = self.try_alloc(rz_sz).unwrap();
        src.data
            .par_chunks(in_strides[0])
            .zip(rz_img.data.par_chunks_mut(rz_strides[0]))
