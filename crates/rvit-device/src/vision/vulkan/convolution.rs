use crate::op_traits::ConvKernel;
...
impl<T: PixelType> ConvKernel<T> for Vulkan
where
    Self: KernelOp<T>,
{
    fn convolution(
        ...
        if width as u32 % HORIZ_TILE_WIDTH as u32 != 0 {
            panic!(
                "kernel width is not a multiple of the tile width {}",
                HORIZ_TILE_WIDTH
            );
        }
        if height as u32 % VERT_TILE_HEIGHT as u32 != 0 {
            panic!(
        ...
        let h_ubo_data = &[width as i32, height as i32, x_kernel.element_count() as i32];
        let h_ubo = self.alloc_ubo_from_slice(h_ubo_data);

        let v_ubo_data = &[width as i32, height as i32, y_kernel.element_count() as i32];
        ...
        let h_work = ComputeWork::new(
            width as u32 / (HORIZ_RESULT_STEPS * HORIZ_TILE_WIDTH) as u32,
            height as u32 / HORIZ_TILE_HEIGHT as u32,
            1,
        );
        let v_work = ComputeWork::new(
            width as u32 / VERT_TILE_WIDTH as u32,
            height as u32 / (VERT_RESULT_STEPS * VERT_TILE_HEIGHT) as u32,
            1,
        );
        ...
        for b in 0..batch_size {
            ...
            for c in 0..channels {
                ...
                let src_img_slice = src.slice(slice_start..slice_end).unwrap();
                ...
                h_program
                    ...
                    .unwrap();
                h_program
                    .try_bind_ssbo::<T>("dst_image", &tmp_img.slice(..).unwrap())
                    .unwrap();
                h_program
                    .try_bind_ssbo::<T>("kernel", &x_kernel.slice(..).unwrap())
                    .unwrap();
                ...
                v_program
                    .try_bind_ssbo::<T>("src_image", &tmp_img.slice(..).unwrap())
                    .unwrap();
                v_program
                    .try_bind_ssbo::<T>("dst_image", &src_img_slice)
                    .unwrap();
                v_program
                    .try_bind_ssbo::<T>("kernel", &y_kernel.slice(..).unwrap())
                    .unwrap();
