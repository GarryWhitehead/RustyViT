...
use crate::vision::op_traits::MakeBorderKernel;
...
impl<T: DataElem, I: BorderMode> MakeBorderKernel<T, I> for Runtime
where
    Self: InterpOp<I>,
{
    fn make_border(
        ...
        let new_image_width = width + 2 * padding;
        let new_image_height = height + 2 * padding;
        let new_shape = [batch_size, channels, new_image_width, new_image_height];
        let new_sz = tensor_size(&new_shape);
        let new_strides = compute_strides(&new_shape);
        let mut mb_image = self.storage.try_alloc(new_sz, T::DTYPE).unwrap();

        let src_slice: &[T] = src.as_slice().unwrap();
        let mb_slice = mb_image.as_mut_slice().unwrap();

        src_slice
            .chunks(src_strides[0])
            .zip(mb_slice.chunks_mut(new_strides[0]))
        ...
        (mb_image, new_shape.to_vec(), new_strides)
    }
}

#[allow(clippy::too_many_arguments)]
impl Runtime {
    fn make_border_kernel<T: Elem, F>(
        &self,
        src: &[T],
        width: usize,
        height: usize,
